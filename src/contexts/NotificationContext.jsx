...
export const NotificationProvider = ({ children }) => {
  ...
  const fetchUnreadCount = async () => {
    try {
      ...
      const response = await fetch('https://shyam-veneer-backend.onrender.com/api/v1/notifications/unreaded', {
        method: 'GET',
        headers: {
  ...
  const fetchAllNotifications = async () => {
    ...
    try {
      ...
      const response = await fetch('https://shyam-veneer-backend.onrender.com/api/v1/notifications/all', {
        signal: controller.signal
      });
  ...
  const markAsRead = async (notificationId) => {
    try {
      const response = await fetch(`https://shyam-veneer-backend.onrender.com/api/v1/notifications/markAsRead/${notificationId}`, {
        method: 'PUT'
      });
