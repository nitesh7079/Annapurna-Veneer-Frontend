...
// Buy/Sell API (kept for backward compatibility with BuySell.jsx and Reports.jsx)
...
export const accountAPI = {
  ...
  getLatestPerPerson: async (filters = {}) => {
    try {
      console.log('Fetching latest transactions per person with filters:', filters);
      const cleanFilters = Object.fromEntries(
        Object.entries(filters).filter(([key, value]) => value !== '' && value !== null && value !== undefined)
      );
      const queryParams = new URLSearchParams(cleanFilters).toString();
      const url = `/accounts/latest-per-person${queryParams ? '?' + queryParams : ''}`;
      console.log('API URL for latest per person:', `${api.defaults.baseURL}${url}`);
      
      const response = await api.get(url);
      console.log('Latest per person response:', response.data);
      return response.data;
    } catch (error) {
      console.error('Failed to fetch latest transactions per person:', error);
  ...
  create: async (transactionData) => {
    try {
      console.log('API call - creating transaction:', transactionData);
